...
import {fetchBlogDetailPage, fetchBlogPage} from "../../api/blog";
...
const BlogDetail = ({ blogSlug }: BlogDetailProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const blogDetailsData = await fetchBlogDetailPage(blogSlug);
        const blogsDataResponse = await fetchBlogPage();
        ...
        setBlogData({
          ...
          posts: blogsDataResponse.posts_list.data.map((post: any) => ({
  ...
  //             />
  //             <div className=" py-2">
  //               {convertDescriptionToElements(post.text[0])}
  //             </div>
  //           </div>
  //         ))}
  //       </div>
