...
interface Service {
    ...
    attributes: {
        ...
        Card: { Title: string; Description: { type: string; children: { text: string; type: string }[] }[]; Photo: { data: any } }[];
        Photo: { data: any };
    };
}
...
const Services = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            console.log(servicesData.services.data[0].attributes.ServiceDescription);
        } catch (error) {
    ...
    return (
        <div>
            ...
            <div className="w-full max-w-[1111px] mx-auto mt-20 max-[1111px]:px-12 max-sm:px-5 max-md:mt-16 mb-32 max-md:mb-28">
                ...
                <div className="grid grid-cols-3 mt-10 gap-20  max-xl:gap-10 max-[1000px]:gap-5 max-[950px]:grid-cols-2
                 max-[500px]:grid-cols-1  ">
                    {services.map(service => (
                        <Link to={`/${slugServices}/${service.attributes.slug}`} key={service.id}>
