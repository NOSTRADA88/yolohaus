    ...
    <div className="flex mt-10 flex-col">
            ...
            <div className="flex flex-col items-start">
              {description.map((item, index) => (
                <div key={index} className="mt-5 ">
                  <div
                    key={index}
                    className={` ${index === 0 ? "bg-lightwhite p-5" : ""}`}
                  >
                    {item.children.map((child, childIndex) => (
                      <p
                        className="font-light text-sm font-museo leading-relaxed text-justify mr-10 max-sm:mr-0"
                        key={childIndex}
                      >
                        {child.text}
                      </p>
      ...
      <div className="grid grid-cols-1 gap-3 mt-5 max-xl:grid-cols-2 max-xl:gap-10 max-sm:grid-cols-1 max-sm:gap-3">
        ...
        <div className="gap-2">
          ...
          <p className="mt-2 font-museo font-light text-sm leading-5 text-maingray">
            Будни: {weekdays}
          </p>
          <p className="mt-1 font-museo font-light text-sm leading-5 text-maingray">
            Выходные: {weekends}
          </p>
        </div>
      </div>
