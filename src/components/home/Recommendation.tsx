      ...
      <div className="grid grid-cols-4 mt-16">
        {recommendations.map((item) => (
          ...
          >
            ...
            bg-gray-800 bg-opacity-70 text-white opacity-0
             group-hover:opacity-100 transition-opacity flex flex-col justify-center items-center p-4 z-10">
              {item.attributes.Description.map((desc, index) => (
                <p key={index} className="mb-1 text-white text-center font-museo font-medium text-xs">{desc.children[0].text}</p>
              ))}
            </div>
          </div>
        ))}
        ...
        <div className="relative p-20 border bg-orange overflow-hidden">
          ...
          <div className="absolute inset-0 flex justify-center items-center pointer-events-none">
            <h2 className="text-center text-white font-museo font-light text-[24px]">Отзывы</h2>
          </div>
        </div>
      </div>
    </div >
